...
- 博文标题下显示 GoatCounter 浏览次数，站点为 `https://zongsoft.goatcounter.com/`。公开计数可能缓存数小时；没有数据、接口失败或脚本被拦截时隐藏数字，不显示虚假的零。计数只在正式环境的 `https://zongsoft.com` 文章页启用，本地预览、搜索页和官网不计数。
