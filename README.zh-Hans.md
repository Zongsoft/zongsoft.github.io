...
- 博文标题下显示 GoatCounter 浏览次数，站点为 `https://zongsoft.goatcounter.com/`。公开计数可能缓存最长 4 小时；接口明确返回零计数（包括尚无访问记录时的 HTTP 404 零计数响应）时显示“0 次浏览”，接口失败、响应无效或脚本被拦截时隐藏数字。计数只在正式环境的 `https://zongsoft.com` 文章页启用，本地预览、搜索页和官网不计数。
